...
impl TxInput {
    ...
    pub fn value<F: TxFetcher>(&self, fetcher: &F) -> Result<u64> {
...
impl Tx {
    ...
    pub fn fee<F: TxFetcher>(&self, fetcher: &F) -> Result<u64> {
...
mod tests {
    ...
    impl TxFetcher for MockFetcher {
        ...
        fn fetch_tx(&self, id: &TxId) -> std::result::Result<Vec<u8>, NetworkError> {
            let id = format!("{}", id);
