...
impl TxInput {
    ...
    pub fn value<F: TxFetcher>(&self, fetcher: &F, network: Network) -> Result<u64> {
        let tx_data = fetcher.fetch_tx(network, &self.prev_tx)?;
...
impl Tx {
    ...
    pub fn fee<F: TxFetcher>(&self, fetcher: &F, network: Network) -> Result<u64> {
        let mut input_value = 0;
        for input in &self.inputs {
            input_value += input.value(fetcher, network)?;
        }
...
mod tests {
    ...
    impl TxFetcher for MockFetcher {
        fn fetch_tx(&self, _: Network, id: &TxId) -> std::result::Result<Vec<u8>, NetworkError> {
            let id = format!("{}", id);
    ...
    fn tx_fee() {
        let bytes = hex!("");
        let tx = Tx::parse(&bytes).unwrap();
        assert_eq!(tx.fee(&MockFetcher::new(), Network::Main).unwrap(), 40000);
        ...
        let tx = Tx::parse(&bytes).unwrap();
        assert_eq!(tx.fee(&MockFetcher::new(), Network::Main).unwrap(), 140500);
    }
